...
### Added
- **문서 업데이트**: README.md, CHANGELOG.md, INSTALL.md 최신화
- **통일된 데이터 스키마**: CashSchema, PositionSchema, TransactionSchema로 모든 Provider 통합
- **Excel 템플릿 개선**: 사용자 친화적인 템플릿 생성 기능 강화
- **날짜별 폴더 구조**: `data/input/YYYY-MM-DD/` 형태의 체계적인 데이터 관리
- **CLI 명령어 개선**: `template` 명령어로 수동 입력 템플릿 생성

### Changed
- **아키텍처 개선**: Provider 패턴 기반의 확장 가능한 구조
- **데이터 플로우 최적화**: 입력 파일 → Provider 파싱 → 스키마 변환 → 데이터 통합 → CSV 내보내기
- **설정 관리 개선**: 통합된 설정 파일 관리 시스템
- **에러 처리 강화**: 상세한 에러 메시지 및 복구 가이드

### Fixed
- **문서 일관성**: 모든 문서의 내용과 구조 통일
- **설치 가이드 개선**: 더 명확한 설치 및 사용법 안내
- **코드 예시 업데이트**: 최신 API에 맞는 예시 코드 제공
