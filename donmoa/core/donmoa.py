...
class Donmoa(LoggerMixin):
    ...
    def _load_account_mappings(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"계좌 매핑 설정 로드 완료: {len(self.account_mappings)}개 타입"
            )
    ...
    def _apply_account_mapping_to_provider(self, provider: BaseProvider) -> None:
        ...
        try:
            ...
            for (
                unified_account,
                provider_mappings_dict,
            ) in self.account_mappings.items():
            ...
            if provider_mappings:
                ...
                self.logger.info(
                    f"{provider.name}에 계좌 매핑 적용 완료: {len(provider_mappings)}개"
                )
    ...
    def export_to_csv(
        ...
        if collected_data is None:
            if self.last_run_result and "collected_data" in self.last_run_result:
                ...
                raise ValueError(
                    "내보낼 데이터가 없습니다. 먼저 데이터를 수집하거나 "
                    "collected_data를 제공해주세요"
                )
        ...
        try:
            ...
            if self.last_run_result:
                ...
                self.last_run_result["exported_files"] = {
                    k: str(v) for k, v in exported_files.items()
                }
    ...
    def run_full_workflow(
        ...
        try:
            ...
            result = {
                ...
                "collection_summary": self.data_collector.get_collection_summary(),
                "export_statistics": self.csv_exporter.get_export_statistics(
                    exported_files
                ),
            }

            self.logger.info(
                f"전체 워크플로우 완료: {workflow_time.total_seconds():.2f}초"
            )
            return result
    ...
    def save_workflow_result(self, output_path: Optional[Path] = None) -> Path:
        ...
        try:
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(
                    self.last_run_result, f, ensure_ascii=False, indent=2, default=str
                )
    ...
    def test_provider_connection(self, provider_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_records": sum(
                    len(data) for data in test_data.values() if isinstance(data, list)
                ),
        ...
        for provider_name, data in collected_data.items():
            ...
            for data_type, records in data.items():
                if not isinstance(records, list):
                    warnings.append(
                        f"Provider '{provider_name}' {data_type}: 잘못된 데이터 형식"
                    )
                    is_valid = False
                    continue
                ...
                if not records:
                    warnings.append(
                        f"Provider '{provider_name}' {data_type}: 빈 데이터"
                    )
                    continue
                ...
                if not isinstance(first_record, dict):
                    warnings.append(
                        f"Provider '{provider_name}' {data_type}: 레코드가 딕셔너리가 아닙니다"
                    )
                    is_valid = False
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            try:
                ...
                provider_health[name] = {
                    ...
                    "message": (
                        "정상"
                        if provider.enabled and provider.credentials
                        else "비활성화됨"
                    ),
                }
            except Exception as e:
                provider_health[name] = {"healthy": False, "message": f"오류: {str(e)}"}
        ...
        if not self.providers:
            health_status["providers"] = {
                "status": {"healthy": True, "message": "등록된 Provider가 없습니다"}
            }
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        metrics["provider_count"] = {"value": total_providers, "unit": "개"}
        metrics["active_providers"] = {"value": active_providers, "unit": "개"}

        # 데이터 메트릭
        if self.last_run_result:
            total_records = sum(
                len(data)
                for data in self.last_run_result.get("collected_data", {}).values()
                if isinstance(data, list)
            )
            metrics["total_records"] = {"value": total_records, "unit": "건"}

        # 메모리 사용량 (간단한 추정)
        import sys

        memory_usage = sys.getsizeof(self) + sum(
            sys.getsizeof(p) for p in self.providers.values()
        )
        metrics["memory_usage"] = {"value": memory_usage / 1024, "unit": "KB"}  # KB
    ...
    def create_backup(self) -> str:
        ...
        backup_data = {
            "timestamp": timestamp,
            "providers": {
                name: provider.get_data_statistics()
                for name, provider in self.providers.items()
            },
