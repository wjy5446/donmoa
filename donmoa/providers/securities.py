...
class MockSecuritiesProvider(BaseProvider, LoggerMixin):
    ...
    def __init__(self, name: str = "MockSecurities", credentials: Optional[Dict[str, str]] = None):
        """
        ...
        while current_date <= end_date:
            ...
            for _ in range(daily_transactions):
                ...
                if transaction_type in ['buy', 'sell']:
                    ...
                    symbol = random.choice(['005930', '000660', '035420', '051910'])  # 삼성전자, SK하이닉스 등
                ...
                transaction = {
                    ...
                    'account': '1234567890',
                    'category': 'stock' if transaction_type in ['buy', 'sell'] else 'cash',
...
class RealSecuritiesProvider(BaseProvider, LoggerMixin):
    ...
    def __init__(self, name: str, credentials: Dict[str, str], endpoints: Dict[str, str]):
        """
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            # 인증 요청
            response = self._make_request('POST', self.endpoints.get('auth', ''), json=auth_data)
            
            if response and 'access_token' in response:
                self.access_token = response['access_token']
                self.refresh_token = response.get('refresh_token')
                self.token_expires_at = datetime.now() + timedelta(hours=1)  # 1시간 후 만료
    ...
    def _refresh_token(self) -> bool:
        ...
        try:
            ...
            response = self._make_request('POST', self.endpoints.get('refresh', ''), json=refresh_data)
